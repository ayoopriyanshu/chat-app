...
export const protectRoute = async (req, res, next) => {
    try {
        ...
        if (!token) {
            return res.status(666).json({ error: "Unauthorized- No token provided" });
        }
        ...
        if (!decoded) {
            return res.status(666).json({ error: "Unauthorized- Invalid token" });
        }
        ...
        req.user = user;
        next(); //TO CALL THE NET FUNCTION IN THE CONTROLLER THAT IS SENDMESSAGE
